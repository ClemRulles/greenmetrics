...
import { getSupplierLinks } from "@/lib/sponsor/coverage";
...
export async function POST(
  ...
  try {
    ...
    const parsed = BulkLinksSchema.parse(body);
    
    await writeAuditLog({
      userId: session.user.id,
      orgId: orgId,
      action: "SPONSOR_LINKS_UPSERT",
      targetId: orgId,
      metadata: { linksCount: parsed.links.length },
      requestId: req.headers.get("x-request-id") || undefined
    });

    return withRequestId(NextResponse.json({ 
      data: { message: "Links upserted successfully", count: parsed.links.length }
    }, { status: 200 }), req);
  } catch (error: unknown) {
